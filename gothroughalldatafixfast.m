...
% % %     the second input to this function is the SD of the filter
    outliersx=isoutlier(xyfixed(:,1),'movmedian',16,'ThresholdFactor',8);
    outlierind=find(outliersx);
...
%     
    ...
    if size(xdata{1,1},2)>size(xdata{2,1},2)
        sac_click=xdata{2,1};
    else
        sac_click=xdata{1,1};
    end
...
%                 end
    ...
    if exist('sacstarts', 'var') ~=0
        ...
        accxateach=accateach(find(accxateach));
        accyateach=accateach(find(accyateach));
...
%         plot(timeaxis_secs(sacends),eyepositiondata(sacends,1),'ro')
    ...
    save(['C:\Users\Lenovo\Documents\matfixed\',fnames{i},'.mat'], 'idformat', 'xyfixed','eyevelxy','eyeaccxy', 'eyeformat', 'tracenumformat', 'msformat', 'ageformat', 'frameshifts_strips_spline', 'outlierind', 'eyepositiondata', 'eyevelmedsmooth', 'eyevelmedgausssmooth', 'timeaxis_secs','blinkframes','sac_click','sacstarts','sacends','sacfailvelthresh','sacfailampthresh','sactooclose')
...
meansacaccx=meansacaccx(find(numsac));meansacaccy=meansacaccy(find(numsac));
numsacraw=numsacraw(find(numsac)); numsac=numsac(find(numsac)); 

xlswrite(filenameforexcel,{'id','eye','tracenum','ms','age','numsac','meanvel','meanamp','numblinks','meanacc','meanvelx','meanvely','meanampx','meanampy','meanaccx','meanaccy'},2,'A1')
xlswrite(filenameforexcel,[idnum',eye',tracenum',ms',age',numsac',meansacpeakvel',meansacamp',numblinks',meanacc',meansacvelx',meansacvely',meansacampx',meansacampy',meanaccx',meanaccy'],2,'A2')